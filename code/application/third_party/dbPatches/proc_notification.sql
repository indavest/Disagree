...
DROP PROCEDURE IF EXISTS `proc_notification` $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `proc_notification`(IN inmemberId VARCHAR(50))
BEGIN
	...
	FETCH jobsqueue INTO jobType;
  		WHILE(NOT loopend) DO
			IF jobType = 0 OR jobType = 1 THEN
              ...
              WHERE argument.memberId LIKE memberCheck AND queue.status = 0 AND ownerProfile.notifyFlag AND queue.type=0 OR queue.type=1;
        ...
        INSERT INTO tmpdata
          SELECT queue.id
					...
					FROM notification_queue AS queue
							...
							JOIN usermember AS owner ON argument.memberId = owner.id
              JOIN usermemberprofile AS ownerProfile ON argument.memberId = ownerProfile.memberId
              WHERE argument.memberId LIKE memberCheck AND queue.status = 0 AND ownerProfile.notifyFlag AND queue.type=2;
        ...
        ELSEIF jobType=3 THEN
				INSERT INTO tmpdata
					...
					FROM notification_queue AS queue
						...
						JOIN usermember AS owner ON argument.memberId = owner.id
            JOIN usermemberprofile AS ownerProfile ON argument.memberId = ownerProfile.memberId
            WHERE argument.memberId LIKE memberCheck AND queue.status = 0 AND ownerProfile.notifyFlag  AND queue.type=3;
      ...
      INSERT INTO tmpdata
        SELECT queue.id
          ...
          WHERE owner.id LIKE memberCheck AND queue.status = 0 AND ownerProfile.notifyFlag AND queue.type=4;
        ...
        INSERT INTO tmpdata
          SELECT queue.id
					...
					FROM notification_queue AS queue
							...
							JOIN usermember AS owner ON argument.memberId = owner.id
              JOIN usermemberprofile AS ownerProfile ON argument.memberId = ownerProfile.memberId
          WHERE owner.id LIKE memberCheck AND queue.status = 0 AND ownerProfile.notifyFlag AND queue.type=6;
      ELSEIF jobType = 7 THEN
        INSERT INTO tmpdata
          ...
          WHERE commenteduser.id LIKE memberCheck AND queue.status = 0 AND commenteduserprofile.notifyFlag AND queue.type=7;
