...
DROP PROCEDURE IF EXISTS `proc_offlinenotification` $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `proc_offlinenotification`()
BEGIN
	...
	FETCH jobsqueue INTO jobType;
      ...
      -- processing Agree / disagree notification
			IF jobType = 0 OR jobType = 1 THEN
              ...
              WHERE queue.status=0 AND ownerProfile.notifyFlag AND (queue.type=0 OR queue.type=1);
        ...
        INSERT INTO tmpdata
          SELECT queue.id
					...
					FROM notification_queue AS queue
							...
							JOIN usermember AS owner ON argument.memberId = owner.id
              JOIN usermemberprofile AS ownerProfile ON argument.memberId = ownerProfile.memberId
              WHERE queue.status = 0 AND ownerProfile.notifyFlag AND queue.type=2;
        ...
        ELSEIF jobType=3 THEN
				INSERT INTO tmpdata
					...
					FROM notification_queue AS queue
						...
						JOIN usermember AS owner ON argument.memberId = owner.id
            JOIN usermemberprofile AS ownerProfile ON argument.memberId = ownerProfile.memberId
            WHERE queue.status = 0 AND ownerProfile.notifyFlag  AND queue.type=3;
      ...
      INSERT INTO tmpdata
        SELECT queue.id
          ...
          WHERE queue.status = 0 AND ownerProfile.notifyFlag AND queue.type=4;
      ...
      ELSEIF jobType = 6 THEN
        INSERT INTO tmpdata
          ...
          WHERE queue.status = 0 AND commenteduserprofile.notifyFlag AND queue.type=6;
        ...
        INSERT INTO tmpdata
          SELECT queue.id
					...
					FROM notification_queue AS queue
							...
							JOIN usermember AS owner ON argument.memberId = owner.id
              JOIN usermemberprofile AS ownerProfile ON argument.memberId = ownerProfile.memberId
          WHERE queue.status = 0 AND ownerProfile.notifyFlag AND queue.type=7;
